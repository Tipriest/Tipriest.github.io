...
我目前用github page + hexo搭建了一个个人的博客框架，本地用Obsidian来写blog，我保存图片最好用什么呢，是用gihub的仓库还是图床，我需要稳定便捷，我平时主要在Windows和Linux上来写Blog，因此需要兼容这两个平台的
...
### **核心方案：GitHub仓库 + PicGo-Core + CDN加速**
#### **1. 创建GitHub图片专用仓库**
   - 新建一个GitHub仓库（如`blog-images`），建议设为公开以使用免费CDN。
   - **优势**：数据完全自主控制，无存储容量限制（适合个人图片存储）。

#### **2. 使用PicGo-Core跨平台上传工具**
   - **工具选择**：  
     - **[PicGo-Core](https://github.com/PicGo/PicGo-Core)**（基于Node.js的命令行工具），兼容Windows/Linux/macOS。  
     - PicGo的 **[官方文档](https://picgo.github.io/PicGo-Doc/zh/guide/)** ，但是已经有6年没有更新了
     - PicGo-Core的 **[官方文档](https://picgo.github.io/PicGo-Core-Doc/zh/guide)** ，但是已经有6年没有更新了
     - 通过npm安装：  
       ```bash
       npm install -g picgo
       # 或使用yarn
   ...
   - **配置GitHub图床**：  
     ...
     2. 运行命令初始化配置：  
        ...
        picgo set uploader
        # 选择「github」并填写以下信息：
        - repo: 用户名/仓库名（如「yourname/blog-images」）
        - branch: main（或其他分支）
        - path: images/（可选子目录）
        - token: 你的GitHub Token
        - customUrl: https://cdn.jsdelivr.net/gh/yourname/blog-images@main
        ```
   - **自动化重命名**：  
     启用「时间戳+随机字符」重命名，避免图片冲突：
     ```bash
     picgo set transformer
     # 选择「path」并设置命名规则（如「{year}/{month}_{hash}」）
     ```
...
![](https://raw.githubusercontent.com/Tipriest/blog-images/main/imgs/my_blog/20250330160542825.jpg)![](https://cdn.jsdelivr.net/gh/Tipriest/blog-images@main/imgs/my_blog/20250330160321978.jpeg)
